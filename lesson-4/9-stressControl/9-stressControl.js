...
function stressContorl(str1, str2) {
  const dictionaryList = str1.split(' ');
  const wordsList = str2.split(' ');
  const dict = {};
  let counter = 0;
  ...
  for (const word of wordsList) {
    const isOneLetterIsUpperCase = 1 === word.split('').filter((l) => l === l.toUpperCase()).length;
    if (!dict.hasOwnProperty(word)) {
      if (!isOneLetterIsUpperCase) {
        counter += 1;
      }
    }
  }
