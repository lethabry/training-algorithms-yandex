...
// Другая черепаха говорит: “Позади меня две черепахи”. Третья черепаха говорит: “Впереди меня две черепахи и позади меня две черепахи”.
...
function whoLied(amount, pairs) {
  let counter = 0;
  for (let i = 0; i < pairs.length; i = i + 2) {
    if (pairs[i] + pairs[i + 1] + 1 === amount) {
      counter++;
    }
  }
  return counter;
