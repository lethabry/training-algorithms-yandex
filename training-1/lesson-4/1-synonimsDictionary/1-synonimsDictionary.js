...
function synonimsDictionary(list, findedWord) {
  const dict = {};
  let ans = '';
  for (const synonims of list) {
    let [word, synonim] = synonims.split(' ');
    dict[word] = synonim;
  }
  for (const [word, sinonim] of Object.entries(dict)) {
    if (findedWord === word) {
      ans = sinonim;
    } else if (findedWord === sinonim) {
      ans = word;
    }
  }
  return ans;
