...
function keyboard(keyPressAmount, allKeyPress) {
  const listKeyPressAmount = keyPressAmount.split(' ').map(Number);
  const listKeyPress = allKeyPress.split(' ').map(Number);
  const dict = {};
  const res = [];
  for (const keyPress of listKeyPress) {
    if (!dict[keyPress]) {
      dict[keyPress] = 1;
    } else {
      dict[keyPress] += 1;
  ...
  for (const [key, value] of Object.entries(dict)) {
    ...
    if (listKeyPressAmount[Number(key) - 1] >= value) {
      res.push('NO');
    } else {
      res.push('YES');
    }
