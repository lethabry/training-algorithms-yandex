...
function subway(data) {
  ...
  if (min1 >= min2) totalmin = min1;
  else if (min2 > min1) totalmin = min2;

  if (max1 >= max2) totalmax = max2;
  else if (max2 > max1) totalmax = max1;

  return totalmin <= totalmax ? [totalmin, totalmax].join(' ') : '-1';
}

process.stdin.on('data', (data) => {
  const result = subway(data);
