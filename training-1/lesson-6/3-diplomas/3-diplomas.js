...
// Так как к бетонной стене прикрепить дипломы достаточно трудно, то он решил купить специальную доску из пробкового дерева, чтобы прикрепить её к стене,
// а к ней — дипломы. Для того чтобы эта конструкция выглядела более красиво, Петя хочет, чтобы доска была квадратной и занимала как можно меньше места на стене.
// Каждый диплом должен быть размещён строго в прямоугольнике размером w на h. Дипломы запрещается поворачивать на 90 градусов.
...
function findOptimalSize(data) {
  const [w, h, n] = data.toString().trim().split(' ').map(Number);
  let l = 0,
    r = Math.max(n * w, n * h);
  while (l < r) {
    let m = Math.floor((l + r) / 2);
    if (checkSize(m, w, h, n)) {
      r = m;
    } else {
      l = m + 1;
    }
...
const checkSize = (size, w, h, n) => {
  const cols = Math.floor(size / w);
  const rows = Math.ceil(n / cols);
  const commonHeight = rows * h;

  return size >= commonHeight;
};

const fs = require('fs');
let fileContent = fs.readFileSync('input.txt', 'utf8');

const result = findOptimalSize(fileContent);
