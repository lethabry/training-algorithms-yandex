...
function largestMultiplication(data) {
  ...
  for (let i = 0; i < array.length; i++) {
    const multiWithMax = max * array[i];
    const multiWithMin = min * array[i];

    if (i === maxIndex || i === minIndex) {
      continue;
    }

    if (multiWithMax > multiRes) {
      res = array[i];
      multiRes = multiWithMax;
    } else if (multiWithMin > multiRes) {
      res = array[i];
      multiRes = multiWithMin;
    }
  }
  return res > 0 ? [res, max] : [min, res];
}
