...
// Горная цепь состоит из отрезков, соединяющих точки на плоскости, каждая из которых находится строго правее предыдущей
// (x-координата следующей точки больше, чем у предыдущей). Трассой на горной цепи называется её часть между двумя фиксированными концами отрезков.
...
function tourism(str1, str2) {
  const mountainTops = str1.split('  ').map((t) => t.split(' ').map(Number));
  const districts = str2.split('  ').map((x) => x.split(' ').map(Number));

  const difference = [];
  const differenceReversed = [];
  const res = [];

  for (let i = 0; i < mountainTops.length - 1; i++) {
    difference.push(mountainTops[i][1] - mountainTops[i + 1][1]);
  }

  for (let i = mountainTops.length - 1; i > 0; i--) {
    differenceReversed.push(mountainTops[i][1] - mountainTops[i - 1][1]);
  }

  for (let i = 0; i < districts.length; i++) {
    const [start, end] = districts[i];

    if (start <= end) {
      const a = sumTops(start - 1, end, difference);
      res.push(a);
    } else {
      const b = sumTops(end - 1, start, differenceReversed);
      res.push(b);
    }
  }
  return res;
