...
function circleLine(data) {
  const [n, i, j] = data.toString().trim().split(/\s+/).map(Number);
  const res1 = Math.max(i, j) - Math.min(i, j) - 1;
  const res2 = Math.min(i, j) + n - Math.max(i, j) - 1;
  return res1 >= res2 ? res2 : res1;
}

process.stdin.on('data', (data) => {
  const result = circleLine(data);
