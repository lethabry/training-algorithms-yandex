...
function takeBlocks(data) {
  const [[length, sumBlocks], blocksList] = data
    .toString()
    .trim()
    .split('\n')
  ...
  let leftBlock = blocksList[0];
  if (blocksList.length === 1) {
    return 0;
  }
  for (let i = 0; i < sumBlocks; i++) {
    if (blocksList[i] < halfLenght) {
      leftBlock = blocksList[i];
    } else if (length % 2 && blocksList[i] === halfLenght) {
      return `${blocksList[i]}`;
    } else {
      return `${leftBlock} ${blocksList[i]}`;
    }
  }
}
...
const result = takeBlocks(fileContent);
